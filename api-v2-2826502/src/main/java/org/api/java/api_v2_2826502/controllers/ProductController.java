...
public class ProductController {
  ...
  public ResponseEntity<?> getProduct(@PathVariable Long id) {
    ...
    if (optProduct.isPresent()) {
      return ResponseEntity
          ...
          .body(servicio.findById(id)); // body es la respueta de los datos que queremos dar alcliente
    } else {
      return ResponseEntity
          .badRequest()
  ...
  public ResponseEntity<?> crearProducto(@Valid @RequestBody Product product,
    ...
    if (bindingResult.hasErrors()) {
      ...
      // return ResponseEntity.badRequest().body("Errores de validacion "); // ARBOLES
      // DE DESICION
  ...
  public ResponseEntity<?> eliminarProducto(@PathVariable Long id) {
    try {
      boolean isEliminar = servicio.eliminarProducto(id);
      if (isEliminar) {
        return ResponseEntity.ok().body("Producto con id : " + id + " Eliminado exitosamente");
      } else {
        return ResponseEntity.badRequest().body("El producto no existe en la base de datos ... ");
      }

    } catch (Exception e) {
      // Manejo de cualquier exepcion
      return ResponseEntity.badRequest().body("Error al eliminar el producto con id : " + id + ". " + e.getMessage());
    }
  }
